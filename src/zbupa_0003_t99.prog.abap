...
CLASS lcl_unit_test DEFINITION
    INHERITING FROM lcl_zbupa_0003
  ...
  PUBLIC SECTION.
    ...
    CONSTANTS:
        gc_default_variant_for_testing TYPE raldb_vari VALUE 'FOR_TESTING'.
  ...
  METHOD testing_report.
    ...
    CALL FUNCTION 'RS_VARIANT_CONTENTS'
      EXPORTING
        ...
        variant = lcl_unit_test=>gc_default_variant_for_testing   " Variant name
      TABLES
...
*  I used this feature to check for any difference between BP and vendor
    ...
    DATA:
      ...
      lv_i         TYPE i,
      lv_kunnr_min TYPE lifnr,
      lv_kunnr_max TYPE lifnr.

    SELECT  MAX( kunnr ) FROM kna1 INTO @lv_kunnr_max
    ...
    TRY.
        ...
        lv_kunnr_min = lo_ran->get_next( ).

      CATCH cx_abap_random. " Exception for CL_ABAP_RANDOM* = 1 max = syst-dbcnt ).
        " Deu Ruim
    ENDTRY.

    UNPACK lv_kunnr_min TO lv_kunnr_min.

    " Mestre de fornecedores (parte geral)
    SELECT SINGLE a1~* FROM kna1 AS a1
    INNER JOIN knbk AS bk ON a1~kunnr = bk~kunnr
    INTO CORRESPONDING FIELDS OF @ls_kna1
        BYPASSING BUFFER
        WHERE a1~kunnr BETWEEN @lv_kunnr_min AND @lv_kunnr_max  .

    IF ls_kna1-stcd1 IS NOT INITIAL.
      ch_s_upload_file-taxnum = ls_kna1-stcd1.
    ELSE.
      ch_s_upload_file-taxnum = ls_kna1-stcd2.
    ENDIF.
