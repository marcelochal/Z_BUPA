...
* Change Control:                                                      *
* Date       | Who                                      |Task          *
* 12.05.2020 | Marcelo Alvares MA004818                 | RITM0080035  *
************************************************************************
* Include for UNIT CLASS METHODS                                       *
*                                                                      *
*&---------------------------------------------------------------------*
*& Include          ZBUPA_0001_T99
*&---------------------------------------------------------------------*


CLASS lcl_unit_test DEFINITION
    INHERITING FROM lcl_zbupa_0001
...
CLASS lcl_unit_test IMPLEMENTATION.
  ...
  METHOD testing_report.
    ...
    APPEND INITIAL LINE TO r_bukrs ASSIGNING FIELD-SYMBOL(<ls_r_bukrs>).
    <ls_r_bukrs>-sign   = 'I'.
    <ls_r_bukrs>-option = 'BT'.
    <ls_r_bukrs>-low    = 'TB01'.
    <ls_r_bukrs>-high   = 'TB18'.
...
**********************************************************************
  ...
  METHOD fill_upload_file.
    ...
    " Fill with new CPF
    <ls_upload_file>-taxnum      = return_random_cpf( ).
    <ls_upload_file>-name1       = |TESTE ABAP UNIT { syst-timlo }1{ syst-datlo } ZBP002|.
    ...
    MOVE-CORRESPONDING me->gt_test_file[ 1 ] TO <ls_upload_file>.
    <ls_upload_file>-taxnum      = return_random_cpf( ).
    <ls_upload_file>-name1       = |TESTE ABAP UNIT { syst-timlo }2{ syst-datlo } ZBP002|.
    CLEAR: <ls_upload_file>-banco, <ls_upload_file>-bankl, <ls_upload_file>-bankn.

    APPEND INITIAL LINE TO me->gt_test_file ASSIGNING <ls_upload_file>.
    MOVE-CORRESPONDING me->gt_test_file[ 1 ] TO <ls_upload_file>.
    <ls_upload_file>-taxnum      = return_random_cpf( ).
    <ls_upload_file>-name1       = |TESTE ABAP UNIT { syst-timlo }3{ syst-datlo } ZBP002|.
